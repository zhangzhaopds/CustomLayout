...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        _pointXArr = [NSMutableArray array];
...
- (void)layoutSubviews {
    ...
    numberOfPoints = [self.dataArray count];
...
- (void)drawGraph {
    ...
    [_pointXArr removeAllObjects];
    for (int i = 0; i < numberOfPoints; i++) {
        
//        float dotValue = [self.delegate valueForIndex:i];
        float dotValue = [self.dataArray[i] floatValue];
        ...
        [_pointYArr addObject:[NSString stringWithFormat:@"%f", circleDot.frame.origin.y]];
        [_pointXArr addObject:[NSString stringWithFormat:@"%f", circleDot.frame.origin.x]];
...
- (float)maxValue {
    ...
    for (int i = 0; i < numberOfPoints; i++) {
        ...
        dotValue = [self.dataArray[i] floatValue];
...
- (void)drawXAxis {
    ...
    for (int i = 0; i < _pointXArr.count; i++) {
        UILabel *laa = [[UILabel alloc] initWithFrame:CGRectMake([_pointXArr[i] floatValue] - 25, [_pointYArr[i] floatValue] - 15, 60, 15)];
        //laa.text = [self.delegate labelOnXAxisForIndex:i];
        laa.text = self.dataArray[i];
        [self addSubview:laa];
        laa.font = [UIFont systemFontOfSize:10];
        laa.textColor = [UIColor whiteColor];
        laa.textAlignment = NSTextAlignmentCenter;
        if (i == 0 || i == _pointXArr.count - 1) {
            laa.hidden = YES;
        }
    }
